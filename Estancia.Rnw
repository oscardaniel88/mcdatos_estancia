...
ggplot(TDC, aes(dia,tdc)) + geom_line()
@
\paragraph{Correlación de variables}~\\
<<Correlación, echo=FALSE, fig.width=20, fig.height=15>>=
...
res$prop_code<-toupper(res$prop_code)
res<-res %>% filter(date_create!="0000-00-00 00:00:00") %>% mutate(date_create=as.Date(date_create),
...
series2018al11ago$eventos <- as.factor(ifelse(as.numeric(series2018al11ago$dia)<=as.numeric(as.Date("2018-08-11")) & as.numeric(series2018al11ago$dia)>=as.numeric(as.Date("2018-03-27")),1,0))
...
mod.CEINS.beta0 <- lm(beta0~AAbeta0+diasem+eventos+tdc+PO+PT,data=datos2018al11agoCEINS)
...
mod.CEINS.beta1 <- lm(beta1~AAbeta1+diasem+eventos+tdc+PO+PT,data=datos2018al11agoCEINS)
...
head(series2018)
##test_index <- as.integer(runif(1,min=0,max=3)*100)
##test <- seq(0:62)
##test <- test -2
##beta0<-series2018[test_index,]$pred.beta0
##beta1<-series2018[test_index,]$pred.beta1
##test2 <- as.integer(exp(beta0+beta1*test))

##result <-as.data.frame(cbind(test,test2))
##ggplot(data=result,aes(x=test,y=test2))+geom_line(colour="red")
@

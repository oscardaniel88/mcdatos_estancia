...
@
~\\
Derivado de este análisis podemos hacer las siguientes conclusiones:
...
\end{itemize}
~\\
Aplicando esta ecuación para una fecha aleatoria:
<<ValidacionModelo, echo=FALSE>>=
detach(package:plyr)
library(dplyr)
#2018-01-17
dias<-series2018 %>% select(dia) %>% 
  filter(dia<='2018-08-11')
index<-sample(1:223,1)
fecha<-dias[index,]
...
ggplot(data=test_result, aes(antelacion,cn_predic))+geom_point()+geom_line(data=datos_reales, aes(x=antelacion,y=cumsum, colour="red"))
@
\section{Resultados}

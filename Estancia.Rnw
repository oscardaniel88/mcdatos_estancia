...
<<pickup_ejemplo, echo=FALSE>>=
historico_grp_rsrv <- historico_ocupacion ###%>% filter(date_in == '2017-05-01')
historico_grp_rsrv <- historico_grp_rsrv %>% group_by(rsrv_code) %>% summarise(nights=sum(nights),
                                                                      date_create = min(date_create),
                                                                      date_in = min(date_in),
                                                                      date_out = max(date_out),
                                                                      count = 1)
historico_grp_rsrv$antelacion<-as.integer(difftime(historico_grp_rsrv$date_in,historico_grp_rsrv$date_create, units="days"))
historico_grp_rsrv$date_create<-as.Date(historico_grp_rsrv$date_create, format = "%Y-%m-%d")
pickup_rsrv <- historico_grp_rsrv %>% group_by(date_create,date_in,antelacion) %>% summarise(cuartos=sum(nights))
pickup_rsrv <- arrange(pickup_rsrv,antelacion)
pickup_rsrv$cuartos<-rev(cumsum(rev(pickup_rsrv$cuartos)))
ggplot(pickup_rsrv, aes(antelacion, cuartos)) + geom_line() + xlim(-1,100) + theme_bw() + theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"))
...
summary(competencia)
competencia <- competencia %>% filter(!nombre %in% c('Holiday Inn Mexico City - Trade Center','Holiday Inn Mexico City-Plaza Universidad','Courtyard by Marriott Mexico City Revolucion') ) 
nombre<-competencia$nombre %>% unique()
Hotel<-c('Hotel1','Hotel2','Hotel3','Hotel4','Hotel5')
comp<-cbind(nombre,Hotel)
comp<-as.data.frame(comp)
competencia <-competencia %>% inner_join(comp)
@
\paragraph{Precios en el tiempo} ~\\
<<PreciosGraf, echo=FALSE>>=
ggplot(competencia, aes(fecha,precio,colour=Hotel)) + geom_line()  + theme_bw() +theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"),legend.position="top",legend.text=element_text(size=15)) 
@
~\\
...
ggplot(competencia, aes(Hotel,precio)) + geom_boxplot(outlier.colour="red", outlier.shape=1,
                outlier.size=2)+ theme(axis.text.x = element_text(angle = 60, hjust = 1))+ theme_bw() +theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"),legend.position="top",legend.text=element_text(size=15)) 
@
~\\
...
datos_reales$version<-c("real")
test_result$version<-c("pronostico")
names(test_result)<-c("antelacion","cumsum","version")
datos_reales <- datos_reales %>% select(antelacion,cumsum,version)
plot_result <- as.data.frame(rbind(datos_reales,test_result))

plots[[i]]<-ggplot(plot_result, aes(antelacion,cumsum,colour=version)) + geom_line()  + theme_bw() +theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"),legend.position="top",legend.text=element_text(size=15)) 

resultados_mape<-test_result[1:29,]
datos_reales<- datos_reales %>% select(antelacion,cumsum)
MAPE_DS<- datos_reales %>% inner_join(resultados_mape,by=c("antelacion"))
MAPE_DS$MAPE<-abs(MAPE_DS$cumsum.x-MAPE_DS$cumsum.y)/MAPE_DS$cumsum.x
...
tp_real <- indicadores %>%  select(fecha, tp) %>% filter(fecha>= '2018-08-14')
...
ggplot(price_result,aes(day,Value, col=Rate_type))+geom_line()+ theme_bw() +theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"),legend.position="top",legend.text=element_text(size=15)) 
#head(pricing_ocup)

price_result %>% spread(Rate_type, Value) %>% mutate(dif = tp_pron-tp) %>% summarise(rev=sum(dif))
@
~\\
...
ggplot(data=prices_tidy,aes(x=day,y=tarifa, col=habs_disp))+geom_line()+ theme_bw() +theme(axis.text=element_text(size=16),
        axis.title=element_text(size=30,face="bold"),legend.position="top",legend.text=element_text(size=15)) 
@
...
library(dplyr)
library(tidyr)
historico_ridge <-read.csv("datos/CEINS_RESERVAS.csv")
dias<-select(historico_ridge,date_in) %>% unique()
antelacion <-as.data.frame(rep(seq(1,27)-2,nrow(dias)))
days<-rep(dias,27)
days<-as.data.frame(days)
days<- days %>% gather("days","value") %>% arrange(value) %>% select(value)
ant<-cbind(days,antelacion)
names(ant)<-c("date_in","antelacion")
ant$nights<-0
historico_ridge$antelacion<-as.integer(difftime(as.POSIXlt(historico_ridge$date_in),as.POSIXlt(as.Date(historico_ridge$date_create),"%Y-%m-%d"), units="days"))
historico_ridge <- historico_ridge %>% arrange(date_in,antelacion)
historico_ridge<-historico_ridge %>% select(rsrv_code,date_in,date_out,antelacion,nights)
historico_ridge <- historico_ridge %>% group_by(date_in,antelacion) %>% summarise(cuartosOcupados=sum(nights))
historico_ridge<-historico_ridge %>% group_by(date_in) %>% mutate(antelacion = (antelacion),
                                                                  cuartosOcupados = rev(cuartosOcupados),
                                                                  acumulado = cumsum(cuartosOcupados))



new_df<-historico_ridge %>%  right_join(ant)

historico_ridge2<-historico_ridge %>% filter(antelacion>=25) %>% group_by(date_in)%>% summarise(cuartosOcupados2=sum(cuartosOcupados))
historico_ridge2$antelacion<-25

new_df<-new_df %>% left_join(historico_ridge2)
new_df[is.na(new_df)] <- 0
new_df$cuartosOcupados<-new_df$cuartosOcupados+new_df$cuartosOcupados2

new_df<-new_df %>% group_by(date_in) %>% mutate(antelacion = rev(antelacion),
                                                                  cuartosOcupados =rev(cuartosOcupados),
                                                                  acumulado =cumsum(cuartosOcupados)) %>% select(date_in,antelacion,acumulado)

new_df<-new_df %>% spread(antelacion,acumulado)
